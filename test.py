...
@click.option('-m', '--model_file', default=Path('model/pipeline.pkl'), type=click.Path(exists=True, file_okay=True, dir_okay=False, readable=True, path_type=Path), help='Save model to file')
def main(
    test_file: Path,
    model_file: Path,
) -> None:
